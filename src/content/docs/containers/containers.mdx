...
<Accordion>
  <AccordionTrigger>The container ID or The hex hash</AccordionTrigger>
  <AccordionContent>
...
| 7   | `-w $HOME`                              | It sets the working directory for the container |
| 8   | `quay.io/almalinuxorg/almalinux:latest` | This shows what image to use for the container.                                          |
| 9   | `sleep infinity`                        | This keeps the container alive by letting it sleep for infinitely long time.             |
...
| :--------------------------- | :------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`:rw`**                    | Access Permission   | **Read-Write (Default).** Allows the container to read, modify, and delete files inside the mount point.                                                  | **Incompatible with:** `:ro`.  **Must be paired with:** `:z` or `:Z` on SELinux hosts to avoid permission errors.                                                             |
| **`:ro`**                    |                     | **Read-Only.** Blocks the container from making any changes. Attempts to write will return a `Read-only file system` error.                               | **Incompatible with:** `:rw`.  **Compatible with:** All SELinux (`:z`, `:Z`), ownership (`:U`), and propagation tags.                                                         |
| **`:z`**                     | SELinux Labeling    | **Shared Relabel.** Recursively changes host file labels to a shared context (`container_file_t`) so multiple containers can access them.                 | **Incompatible with:** `:Z`.  **Compatible with:** `:rw`, `:ro`, `:U`, and propagation tags.                                                                                  |
| **`:Z`**                     |                     | **Private Relabel.** Recursively changes host file labels to a unique, exclusive context. Only _this_ container can access the files.                     | **Incompatible with:** `:z`.  **Compatible with:** `:rw`, `:ro`, `:U`, and propagation tags.                                                                                  |
...
| **`:O`**                     | Performance Overlay | **Overlay Mount.** Mounts the host directory as a read-only base layer with a temporary write layer. Changes are lost when the container stops.           | **Incompatible with:** `:z`, `:Z` (bypasses SELinux labeling entirely), and propagation tags.  **Note:** Implicitly acts as `:rw` internally while keeping the host pristine. |
| **`:private` / `:rprivate`** | Mount Propagation   | **Private (Default).** Mount changes inside the container do not show up on the host, and vice versa. (`r` applies recursively).                          | **Incompatible with:** Other propagation tags and `:O`.  **Compatible with:** All access (`:rw`/`:ro`), SELinux (`:z`/`:Z`), and `:U` tags.                                   |
| **`:shared` / `:rshared`**   |                     | **Shared.** Two-way propagation. Mounts made on the host reflect inside the container, and mounts made inside the container reflect on the host.          | **Incompatible with:** Other propagation tags and `:O`.  **Compatible with:** All access, SELinux, and `:U` tags.                                                             |
| **`:slave` / `:rslave`**     |                     | **Slave.** One-way propagation. Mounts made on the host reflect inside the container, but container mounts do not show up on the host.                    | **Incompatible with:** Other propagation tags and `:O`.  **Compatible with:** All access, SELinux, and `:U` tags.                                                             |
| **`:unbindable`**            |                     | **Unbindable.** Prevents this specific directory from ever being cloned or bind-mounted somewhere else in the future.                                     | **Incompatible with:** Other propagation tags and `:O`.  **Compatible with:** All access, SELinux, and `:U` tags.                                                             |


9. `-w $HOME`

It sets the which directory to default to when opening the container. Host home directory is passed as a volume and named as same as the host home name and `$HOME` passes the same name so the container use it as the default working directory.
8.`quay.io/almalinuxorg/almalinux:latest`
...
_sh as the shell is used to enter into container here but, you can use any Linux shell of your favourite._
...
It is not necessary to enter the container to execute a command. A command inside the container can be directly executed without going inside. (ie. install a package or update the system running inside the container). The podman maps both the container user and its root to uids owned by host user therefor, it is needed to specify root when entering the container to perform tasks such as updating the system or installing packages inside the container.

```bash
podman exec -it --user root xyz whoami
```
```bash
root
```
...
Next up, we will be seeing how to make a Capsule container using Podman that works as if its the host with the tightest integration possible without degrading security measures.
